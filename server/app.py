...
# def process_image_and_detect(image: Image.Image):
...
#     # Select detection with highest confidence score
#     if len(scores) == 0:
#         raise HTTPException(status_code=404, detail="No valid damage detections found")
...
#     # Adjust confidence score if below threshold
#     confidence_score = max(selected_score, 1 - selected_score) if selected_score < 0.4 else selected_score
...
# @app.post("/predict", summary="Detect damage from image URL")
# async def predict(request: ImageRequest):
#     """Detect road damage from an image URL"""
#     try:
#         logger.info(f"Downloading image from: {request.image_url}")
#         response = requests.get(request.image_url)
#         if response.status_code != 200:
...
#         image = Image.open(io.BytesIO(response.content)).convert("RGB")
#         return process_image_and_detect(image)
...
# @app.post("/predict-file", summary="Detect damage from uploaded image")
# async def predict_file(file: UploadFile = File(..., description="Image file to analyze")):
#     """Detect road damage from an uploaded image file"""
#     try:
